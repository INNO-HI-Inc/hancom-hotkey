...
### Changed (BREAKING)
- **트리거 키를 ⌘ → ⌃로 변경**: `⌃0~4 → ⌃1~5` (이전: `⌘0~4 → ⌃1~5`)
  - 한컴 Mac은 원래 `⌃1~0`로 스타일을 부여하므로, 본 앱도 동일한 ⌃ 시리즈로 통일
  - `⌃0`이 1번째 슬롯(바탕글)을 가리켜 0-인덱스 멘탈 모델 제공
  - 단점: 한컴 네이티브 `⌃1=바탕글`을 직접 누르던 사용자는 이제 `⌃0`을 사용해야 함
- schema v6 마이그레이션: 기존 v0.5.0 사용자는 새 기본값으로 자동 초기화
